...
const GENRES1 = props => {
  const {first} = props
  const {name} = first
  return <li className="listparacolor">{name}</li>
}

const SPOKEN = props => {
  const {first} = props
  const {englishName} = first
  return <li className="listparacolor">{englishName}</li>
}
    ...
    return (
      <>
        ...
        <div className="Indepthsecondcard">
          <ul className="genre">
            <p className="paracolor">Genres</p>
            {genres.map(each4 => (
              <GENRES1 first={each4} key={each4.id} />
            ))}
          </ul>
          <ul className="genre">
            <p className="paracolor">Audio Available</p>
            {spokenLanguages.map(each4 => (
              <SPOKEN first={each4} key={each4.id} />
            ))}
          </ul>
          <div className="genre">
            <p className="paracolor">Rating Count</p>
            <p className="listparacolor">{voteAverage}</p>
            <p className="paracolor">Rating Average</p>
            <p className="listparacolor">{voteCount}</p>
          </div>
          <div className="genre">
            <p className="paracolor">Budget</p>
            <p className="listparacolor">{budget}</p>
            <p className="paracolor">Release Date</p>
            <p className="listparacolor">{releaseData}</p>
          </div>
        </div>
        <h1 className="bottom-head">More Like these</h1>
